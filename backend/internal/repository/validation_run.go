...
func (repository *ValidationRunRepository) Get(id uint) (model.ValidationRun, error) {
	...
	if err := repository.preload(repository.db).First(&run, id).Error; err != nil {
		return run, fmt.Errorf("get validation run: %w", err)
	}
	return run, nil
}

// GetForUpdate loads the run with its motion program and waivers while taking a
// row lock so concurrent waiver grants cannot create duplicates. SQLite has no
// FOR UPDATE and treats the locking clause as a no-op; its single-writer model
// serializes transactions instead.
func (repository *ValidationRunRepository) GetForUpdate(id uint) (model.ValidationRun, error) {
	var run model.ValidationRun
	query := repository.preload(repository.db.Clauses(clause.Locking{Strength: "UPDATE"}))
	if err := query.First(&run, id).Error; err != nil {
		return run, fmt.Errorf("get validation run for update: %w", err)
	}
	return run, nil
}

func (repository *ValidationRunRepository) preload(query *gorm.DB) *gorm.DB {
	return query.Preload("MotionProgram").Preload("Waivers", func(db *gorm.DB) *gorm.DB {
		return db.Order("granted_at ASC, id ASC")
	})
}
...
func (repository *ValidationRunRepository) List(page, pageSize int, programID uint, status string) ([]model.ValidationRun, int64, error) {
	...
	if err := repository.preload(query).Order("started_at DESC, id DESC").Offset((page - 1) * pageSize).Limit(pageSize).Find(&runs).Error; err != nil {
...
func (repository *ValidationRunRepository) FindByIdempotencyKey(key string) (model.ValidationRun, error) {
	var run model.ValidationRun
	if err := repository.preload(repository.db).Where("idempotency_key = ?", key).First(&run).Error; err != nil {
...
func (repository *ValidationRunRepository) LatestByInput(inputHash, algorithmVersion string) (model.ValidationRun, error) {
	var run model.ValidationRun
	if err := repository.preload(repository.db).Where("input_hash = ? AND algorithm_version = ?", inputHash, algorithmVersion).
...
// ActiveWaiver returns the unexpired waiver for a finding, or
// gorm.ErrRecordNotFound when none exists.
func (repository *ValidationRunRepository) ActiveWaiver(runID uint, kind string, index int, now time.Time) (model.ViolationWaiver, error) {
	var waiver model.ViolationWaiver
	err := repository.db.Where("validation_run_id = ? AND finding_kind = ? AND finding_index = ? AND expires_at > ?", runID, kind, index, now).
		Order("id DESC").First(&waiver).Error
	return waiver, err
}

func (repository *ValidationRunRepository) CreateWaiver(waiver *model.ViolationWaiver) error {
	if err := repository.db.Create(waiver).Error; err != nil {
		return fmt.Errorf("create violation waiver: %w", err)
	}
	return nil
}
...
// Accept moves a run that is either reviewed or waiver-complete (failed with
// every violation waived) to accepted in one guarded UPDATE.
func (repository *ValidationRunRepository) Accept(id uint, reviewer uint, note string) error {
	result := repository.db.Model(&model.ValidationRun{}).
		Where("id = ? AND validation_status IN ?", id, []string{"reviewed", "failed"}).
		Updates(map[string]any{"validation_status": "accepted", "reviewed_by": reviewer, "reviewed_at": gorm.Expr("CURRENT_TIMESTAMP"), "review_note": note})
	if result.Error != nil {
		return fmt.Errorf("accept validation run: %w", result.Error)
	}
	if result.RowsAffected != 1 {
		return ErrStateConflict
	}
